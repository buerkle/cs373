...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        # ADD CODE HERE
        angle = motion[0]
        dist = motion[1] + random.gauss(0.0, self.distance_noise)
        beta = (dist / self.length) * tan(angle)
        if abs(beta) < 0.001:
            x = self.x + dist * cos(self.orientation)
            y = self.y + dist * sin(self.orientation)
        else:
            R = self.length / tan(angle)
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            x = cx + sin(self.orientation + beta) * R
            y = cy - cos(self.orientation + beta) * R

        orientation = (self.orientation + beta + random.gauss(0, self.steering_noise)) % (2 * pi)
    ...
    def sense(self, noise=1): #do not change the name of this function
        ...
        for landmark in landmarks:
            global_orientation = atan2(landmark[0] - self.y, landmark[1] - self.x) % (2*pi)
            measurement = global_orientation - self.orientation
            if noise == 1:
                measurement += random.gauss(0, self.bearing_noise)
            Z.append(measurement % (2*pi))
